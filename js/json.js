    ...
    $.get("request.php?op=returnAnswerList&question_id="+currentQuestionId, function (data) {  //request data
        ...
        for (var i = 0; i < 9; i++) {  // load the answers
            if( currentQuestionId ==1){
                ...
                var listgot = document.getElementById(i);
            }
            ...
            //listgot.click( clickList(listgot, json[i]),currentQuestionId ); // if uncomment this, it would automatically run without click???
            //listgot.addEventListener("click",clickList(listgot, json[i]) );
        }//for

    var rightanswer= document.getElementById("right");   //it is so weird....2 dimension array if use jquery...
    console.log("rightanswer");
    console.log(rightanswer);
    rightanswer.addEventListener("click", clickAnswer( rightanswer ),false );
...
function clickList ( listgot, json, currentQuestionId ) {
    console.log("json");
    console.log(json);
    $(".rightAnswer")[0].style.background = "url(json['answer_img'])";     // to the right answer
    console.log( $(".rightAnswer")[0] );
    console.log(listgot);
    listgot.style.background = "#ffffff";
    //right show yes.  else show no/
    if (json['answer_right'] == 1) if (confirm("Right Answer. Click to Next.")) {
        currentQuestionId++;
        loadVideo( currentQuestionId );
        loadQuestions( currentQuestionId );
    }
    else
        $(".rightAnswer")[0].className = "rightAnswerOutline";    //change the style if it is wrong
}
